...
local Base = import("rbxassetid://14217530095").Base
...
local customUpvalueChange = ContextMenuButton.new("rbxassetid://5458573463", "Change Custom Upvalue")
...
local customUpvalueInner = customUpvalue.Instance.Inner
local customUpvalueContent = customUpvalueInner.Content
local customUpvalueButtons = customUpvalueInner.Buttons.SetCancel
local customUpvalueName = customUpvalueContent.UpvalueName.Input
local customUpvalueType = customUpvalueContent.Type
local customUpvalueValue = customUpvalueContent.Value.Input

local upvalueTypeDropdown = Dropdown.new(modifyUpvalueType)
local elementTypeDropdown = Dropdown.new(modifyElementType)
local customUpvalueTypeDropdown = Dropdown.new(customUpvalueType)
...
customUpvalueTypeDropdown:SetCallback(typeDropdownAdjust)

local function generateScriptFormat(elementIndex)
    local generatedScript = [[-- Generated by Hydronium's Upvalue Scanner: https://github.com/nunera/Hydronium

local aux = loadstring(game:HttpGetAsync("https://raw.githubusercontent.com/nunera/Hydronium/revision/ohaux.lua"))()
